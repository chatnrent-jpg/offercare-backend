"""pgvector provider embeddings + Stripe instant pay retention tables."""
...
revision: str = "018_pgvector_instant_pay"
down_revision: Union[str, None] = "017_md_outreach_facility_contact"
...
def _pgvector_available(bind) -> bool:
    bind.execute(text("SAVEPOINT vetted_pgvector_probe"))
    try:
        bind.execute(text("CREATE EXTENSION IF NOT EXISTS vector"))
        version = bind.execute(text("SELECT extversion FROM pg_extension WHERE extname = 'vector'")).scalar()
        bind.execute(text("RELEASE SAVEPOINT vetted_pgvector_probe"))
        return bool(version)
    except Exception:
        bind.execute(text("ROLLBACK TO SAVEPOINT vetted_pgvector_probe"))
        return False
...
def downgrade() -> None:
    ...
    if "provider_profile_embeddings" in tables:
        op.execute("DROP TABLE IF EXISTS provider_profile_embeddings")
