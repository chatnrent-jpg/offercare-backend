...
from app.seed import seed_de_nursing_home_demo
...
def test_de_nursing_home_seed_creates_post_acute_demo(db: Session) -> None:
    payload = seed_de_nursing_home_demo(db)
    assert payload["state"] == "DE"
    assert payload["facility_type"] == "NURSING_HOME"
    assert payload["offer_id"]
    assert payload["provider_ids"]


def test_de_nursing_home_seed_ranks_cna_for_gna_shift(db: Session) -> None:
    payload = seed_de_nursing_home_demo(db)
...
def test_de_cna_sees_matched_gna_shift(db: Session) -> None:
    from app.models import MarylandProvider

    payload = seed_de_nursing_home_demo(db)
    provider = (
        db.query(MarylandProvider)
        .filter(MarylandProvider.email == "de.snf.cna.a@offercare.demo")
        .first()
    )
...
def test_de_nursing_home_seed_endpoint(client: TestClient) -> None:
    response = client.post("/api/seed/de-nursing-home")
...
def test_admin_dashboard_includes_de_nursing_home_seed(client: TestClient) -> None:
    html = client.get("/admin")
    assert html.status_code == 200
    assert "seed-de-nursing-home-btn" in html.text
    js = client.get("/admin/app.js")
    assert "/api/seed/de-nursing-home" in js.text
